 ...
 *
 * Paddle's API is JSON, so requests go out through HttpClient::postJson(), and every
 * response is wrapped in a `{"data": …}` envelope that send() unwraps so the rest of this
 * class reads the entity directly.
 *
...
final class PaddleCheckout extends Checkout
{
    ...
    private function assertCheckoutMode(): void
    {
        ...
        throw new CheckoutException($configured === 0
            ? 'This PaddleCheckout was constructed without a checkout URL, so there is nowhere to send the '
                . 'customer. Pass either $hostedCheckoutUrl — the link copied from Paddle > Checkout > Hosted '
                . 'checkout — or $paymentPageUrl, your own approved page running Paddle.js.'
            : 'This PaddleCheckout was constructed with both $hostedCheckoutUrl and $paymentPageUrl. A customer '
                . 'goes to one checkout or the other, so pass exactly one.');
    }
     ...
     *
     * No `billing_cycle` is ever sent: its absence is what makes a price one-time rather
     * than a subscription, and Checkout deals only in one-time payments.
     *
     * @return list<array<string, mixed>>
     */
    private function itemParams(CheckoutRequest $request): array
    {
        if ($request->lineItems === []) {
            return [$this->itemParam($request->reference, $request->amount, 1)];
        }

        return array_map(
            fn ($item): array => $this->itemParam($item->description, $item->unitPrice, $item->quantity),
            $request->lineItems
        );
    }
     ...
     */
    private function itemParam(string $description, Money $unitPrice, int $quantity): array
    {
        return [
            'quantity' => $quantity,
            'price' => [
     ...
     * @param array<string, mixed> $transaction
     */
    private function mapTransactionStatus(array $transaction): TransactionStatus
    {
    ...
    private function sendRaw(string $method, string $path, array $params, int $timeoutSeconds): array
    {
        ...
        $response = $method === 'GET'
            ? $client->get($params === [] ? $uri : $uri . '?' . http_build_query($params), $headers)
            : $client->postJson($uri, $params, $headers);
